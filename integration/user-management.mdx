...
Long-lived opaque API keys (`pmth_<hex>`, presented as `app_<clientId>.pmth_<hex>`) can be issued per-user and exchanged for a **`SignerSession`** without repeating device login.
