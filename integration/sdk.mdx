...
| `@livepeer/clearinghouse-identity-webhook/protocol` | Node | `handleAuthorize`, go-livepeer webhook wire protocol |
| `@livepeer/clearinghouse-identity-webhook/legacy-env` | Node | `createLegacyWebhookConfigFromEnv` — maps `JWT_*` / `CLAIM_*` to OIDC verifier |
| `@livepeer/clearinghouse-identity-webhook/verifiers` | Node | `createOidcVerifier`, `createApiKeyVerifier`, `createEndUserVerifierFromEnv` |
