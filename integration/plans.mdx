...
<Note>
  `is_network_default` cannot be set on custom plans. The Network Price plan (the discovery/manifest plan) is managed separately via `PUT .../manifest`.
</Note>
...
Body must include **`id`** (plan UUID). Omitted fields keep existing values. If **`capabilities`** is present, it **replaces** all bundles for that plan; omit the key to leave bundles unchanged. `PUT` on the Network Price plan id returns `400` — edit exclusions via `PUT .../manifest` or the Plans UI.
