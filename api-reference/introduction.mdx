...
All endpoints enforce that the authenticated M2M client belongs to the same app as the `{clientId}` in the URL path. Mismatches return **`404 Not Found`** — not `401` or `403` — to prevent app id enumeration.
